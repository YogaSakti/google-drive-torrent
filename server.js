...
app.get('/dashboard', (req, res) => {
    ifLoggedIn(req, res, () => {
        ...
        console.log(user)
        const displayName = (json.has(user.name.displayName)) ? user.name.displayName : "name";
        const givenName = (json.has(user.name.givenName)) ? user.name.givenName : "name";
...
app.post('/add-torrent', (req, res) => {
    ifLoggedIn(req, res, () => {
        ...
        torrent.on('download', (bytes) => {
            // console.log(`${torrent.name} downloaded ${torrent.downloaded / 1024} kB \(${torrent.progress * 100}%) @ ${torrent.downloadSpeed / 1024} kB/s. \ETA: ${torrent.timeRemaining / 1000} sec`);
        });
    });    
});
